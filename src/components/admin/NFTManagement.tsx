...
export const NFTManagement: React.FC<NFTManagementProps> = ({
  ...
  useEffect(() => {
    const initializeContract = async () => {
      ...
      try {
        console.log("Initializing contract with address:", F8_ADDRESS);
  ...
  const handleUpdateMission = async (missionId: string) => {
    ...
    try {
      ...
      const expiryTimestamp = Math.floor(
        new Date(editForm.expiryDate).getTime() / 1000
      );
  ...
  const startEditingMission = (mission: MissionData) => {
    ...
    setEditForm({
      ...
      expiryDate: new Date(mission.expiryDate.toNumber() * 1000)
        .toISOString()
        .split("T")[0],
    });
  };
  ...
  const handleClaimReward = async (missionId: string, tokenId: string) => {
    if (!f8Contract) return;
                  ...
                  <div className="bg-slate-700/50 rounded-lg p-6 border border-purple-500/50">
                    ...
                    <div className="mt-4 flex justify-end">
                      <button
                        ...
                        disabled={
                          !newMission.missionName ||
                          !newMission.missionAmount ||
                          !newMission.rebornAmount ||
                          !newMission.expiryDate
                        }
                ...
                <div className="grid gap-6">
                  {missions.map((mission) => (
                    <div
                      ...
                      className="bg-slate-700/50 rounded-lg p-6 border border-slate-600 hover:border-purple-500/50 transition-all duration-200"
                    >
                      {editingMission === mission.missionId ? (
                        ...
                        <div className="space-y-4">
                          ...
                          <div className="flex justify-end gap-2">
                            ...
                            <button
                              onClick={() =>
                                handleUpdateMission(mission.missionId)
                              }
                              disabled={
                                !editForm.missionName ||
                                !editForm.missionAmount ||
                                !editForm.rebornAmount ||
                                !editForm.expiryDate
                              }
                              className="bg-purple-500 hover:bg-purple-600 disabled:bg-purple-500/50 disabled:cursor-not-allowed px-4 py-2 rounded-lg font-medium text-white text-sm"
                            >
                        ...
                        <>
                          <div className="flex justify-between items-start">
                            ...
                            <div className="flex items-center gap-2">
                              ...
                              {mission.canClaim && selectedTokenId && (
                                <button
                                  onClick={() =>
                                    handleClaimReward(
                                      mission.missionId,
                                  ...
                                  className="bg-purple-500 hover:bg-purple-600 px-3 py-1 rounded-lg text-white text-sm"
                                >
                                  Claim Reward
                                </button>
                              )}
